...
| `Del` | Авто-убийство врагов | сам вызывает `EnemyRobot.TakeDamage` каждые 0.25 с |
...
| `F9` | +10000 золота |
| `F10` | +1 уровень |
| `F11` | убить всех врагов и объекты на карте (разово) |
| `F12` | полное лечение |
...
То же, что `F11`, только само: пока тумблер включён, мод раз в 0.25 секунды проходит по
`EnemyRegistry.All` и добивает всё живое. Враги умирают почти в момент появления, золото, души
и опыт капают как от обычного убийства — урон наносится через родной `EnemyRobot.TakeDamage`,
а не через `Die()`.

Два отличия от разового `F11`:

- **Разрушаемые объекты не трогает.** Ящики и прочий `IsProp` авто-режим пропускает, иначе
  вся сцена лопалась бы в момент загрузки уровня, с соответствующим фейерверком эффектов.
  `F11` по-прежнему сносит и их.
- **Работает по таймеру, а не каждый кадр.** Обход реестра через IL2CPP-интероп 200 раз в
  секунду — это чистые накладные расходы, а четверть секунды на глаз неотличима от мгновенного.

Счётчик убитых показывается в оверлее и обнуляется при каждом включении тумблера. В сетевой
сессии авто-убийство гасится вместе с остальными читами.
